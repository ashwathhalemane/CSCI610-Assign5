  ...
  var myCylinder = null;
  var myCone = null;
  var myCubeBuffer = null;
  var myCylinderBuffer = null;
...
function drawShapes() {
    ...
    if(myCubeBuffer == null) myCubeBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, myCubeBuffer);

    if(myCylinderBuffer == null) myCylinderBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, myCylinderBuffer);

    // console.log(myCubeBuffer)

    let modelMatrix = glMatrix.mat4.create();
    // let modelMatrixForCube = glMatrix.mat4.create();
    let modelMatrixForCylinder = glMatrix.mat4.create();

    // drawing the teapot rotating around Y  180 degrees
    glMatrix.mat4.rotateY (modelMatrix,  modelMatrix, radians(-180.0))
    // glMatrix.mat4.rotateY (modelMatrixForCube, modelMatrixForCube, radians(0.0))
    glMatrix.mat4.rotateY (modelMatrixForCylinder, modelMatrixForCylinder, radians(-180.0))
    // send the model matrix to the shader and draw.
    gl.uniformMatrix4fv (program.uModelT, false, modelMatrix);
    // gl.uniformMatrix4fv (program.uModelT, false, modelMatrixForCube);
    gl.uniformMatrix4fv (program.uModelT, false, modelMatrixForCylinder);
